...
class SpatialPartitioningModel:
    ...
    def update_model(self, dt: float, mouse_pos: Vector2) -> None:
        ...
        update_cell_entity_list: list[Tuple[int, int, GameEntity]] = []
        for row in range(self.grid_height):
            for col in range(self.grid_width):
                for i in range(len(self.grid_space[row][col]) - 1, -1, -1):
                    ...
                    if new_r != row or new_c != col:
                        ...
                        # Track which entities moved grid squares. We don't want to add them to the new grid square until we've updated the position of all other entities
                        update_cell_entity_list.append((new_r, new_c, e))
        for cell_entity in update_cell_entity_list:
    ...
    def apply_forces_to_entity(self, entity: GameEntity, mouse_pos: Vector2) -> None:
        ...
        for dr in range(-cell_range, cell_range + 1):
            for dc in range(-cell_range, cell_range + 1):
                # TODO update wrapping behavior to make sense with flocking (or update flocking to know how to wrap)
                grid_r: int = r + dr
